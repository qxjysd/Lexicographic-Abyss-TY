...
public class InventoryPane extends Component {
	...
	protected void layout() {
		...
		for (InventorySlot i : equipped){
			int row = slotCount / 5;
			int col = slotCount % 5;
			i.setRect(x+4 + col*(SLOT_WIDTH+1), y+4 + row*(SLOT_HEIGHT+1), SLOT_WIDTH, SLOT_HEIGHT);
			slotCount++;
		}
	...
	public void updateInventory(){
		...
		equipped.get(3).item(stuff.misc[0] == null ? new WndBag.Placeholder( ItemSpriteSheet.SOMETHING ): stuff.misc[0]);
		equipped.get(4).item(stuff.misc[1] == null ? new WndBag.Placeholder( ItemSpriteSheet.SOMETHING ): stuff.misc[1]);
		equipped.get(5).item(stuff.misc[2] == null ? new WndBag.Placeholder( ItemSpriteSheet.SOMETHING ): stuff.misc[2]);
		equipped.get(6).item(stuff.misc[3] == null ? new WndBag.Placeholder( ItemSpriteSheet.SOMETHING ): stuff.misc[3]);
		equipped.get(7).item(stuff.ring[0] == null ? new WndBag.Placeholder( ItemSpriteSheet.RING_HOLDER ): stuff.ring[0]);
		equipped.get(8).item(stuff.ring[1] == null ? new WndBag.Placeholder( ItemSpriteSheet.RING_HOLDER ): stuff.ring[1]);
		equipped.get(9).item(stuff.ring[2] == null ? new WndBag.Placeholder( ItemSpriteSheet.RING_HOLDER ): stuff.ring[2]);
	...
	public static void useTargeting(){
		...
		} else if (targetingSlot != null){
			// reasonix: 有物品槽但无目标时显示锁定框
			targeting = true;
			crossB.point(targetingSlot.sprite.center(crossB));
			crossB.visible = true;
		} else {
		...
		protected void onClick() {
			...
			if (targeting){
				if (targetingSlot == this){
					// reasonix: lastTarget为null时取消瞄准，避免autoAim后访问lastTarget.pos空指针
