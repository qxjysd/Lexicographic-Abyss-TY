			...
			if (spiritForm != null){
				RedButton btnSpirit = new RedButton(Messages.get(WndUseTrinity.class, "spirit",
					...
					protected void onClick() {
						...
						for (Ring r : Dungeon.hero.belongings.ring()){
							if (r != null && r.getClass().equals(spiritForm.getClass())){
								hasClass = true;
								break;
							}
						}
						if (!hasClass){
							for (KindofMisc m : Dungeon.hero.belongings.misc()){
								if (m != null && m.getClass().equals(spiritForm.getClass())){
									hasClass = true;
									break;
								}
							}
