	...
	public static void onTalentUpgraded( Hero hero, Talent talent ){
		...
		if (talent == THIEFS_INTUITION && hero.pointsInTalent(THIEFS_INTUITION) == 2){
			for (Ring r : hero.belongings.ring){
				if (r != null && !ShardOfOblivion.passiveIDDisabled()){
					r.identify();
				}
			}
			for (KindofMisc m : hero.belongings.misc){
				if (m instanceof Ring && !ShardOfOblivion.passiveIDDisabled()){
					m.identify();
				}
			}
		...
		if (talent == THIEFS_INTUITION && hero.pointsInTalent(THIEFS_INTUITION) == 1){
			for (Ring r : hero.belongings.ring){
				if (r instanceof Ring) r.setKnown();
			}
			for (KindofMisc m : hero.belongings.misc){
				if (m instanceof Ring) ((Ring) m).setKnown();
			}
		}
