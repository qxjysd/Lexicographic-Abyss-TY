...
import com.shatteredpixel.shatteredpixeldungeon.windows.WndQuiz;
...
public class Hero extends Char {
	...
	public void updateHT( boolean boostHP ){
		...
		if (Dungeon.traits != null) {
			float hpBonus = TraitHandler.getMaxHPBonus(this);
			if (hpBonus != 0) {
				HT = Math.round(HT * (1 + hpBonus));
			}
		}
	...
	private static final String DODGE		= "dodgeSkill";
	...
	public void restoreFromBundle( Bundle bundle ) {
		...
		dodgeSkill = bundle.getFloat( DODGE );
	...
	public int defenseSkill( Char enemy ) {
		...
		float evasion = defenseSkill + dodgeSkill;
		
		evasion *= RingOfEvasion.evasionMultiplier( this );

		// 词条闪避加成（DODGE + DEX）
		if (Dungeon.traits != null) {
			float dodgeBonus = TraitHandler.getDodgeBonus(this);
			if (dodgeBonus != 0) {
				evasion *= (1 + dodgeBonus);
			}
		}
	...
	public float speed() {
		...
		// 词条速度加成（SPD）
		if (Dungeon.traits != null) {
			float spdBonus = TraitHandler.getSpeedBonus(this);
			if (spdBonus != 0) {
				speed *= (1 + spdBonus);
			}
		}
	...
	public float attackDelay() {
		...
		if (!RingOfForce.fightingUnarmed(this)) {
			...
			float baseDelay = delay * belongings.attackingWeapon().delayFactor( this );
			
			// 词条攻速加成（HASTE，降低攻击延迟）
			if (Dungeon.traits != null) {
				float hasteBonus = TraitHandler.getHasteBonus(this);
				if (hasteBonus != 0) {
					baseDelay /= (1 + hasteBonus);
				}
			}
	...
	public int attackProc( final Char enemy, int damage ) {
		...
		// Artifact on-attack hooks
		Item artifactItem = belongings.artifact;
				if (artifactItem != null && !artifactItem.cursed && buff(MagicImmune.class) == null && damage > 0) {
					if (artifactItem instanceof ChaliceOfBlood) {
						damage = ((ChaliceOfBlood) artifactItem).onHeroAttack(this, enemy, damage);
					} else if (artifactItem instanceof MasterThievesArmband) {
				...
				for (KindofMisc miscItem : belongings.misc) {
					if (miscItem != null && !miscItem.cursed && buff(MagicImmune.class) == null && damage > 0) {
						if (miscItem instanceof ChaliceOfBlood) {
							damage = ((ChaliceOfBlood) miscItem).onHeroAttack(this, enemy, damage);
						} else if (miscItem instanceof MasterThievesArmband) {
		...
		// 词条效果加成
		if (Dungeon.traits != null) {
			damage = Math.round(damage * (1 + TraitHandler.getAttackBonus(this)));
			damage += TraitHandler.onAttackProc(this, enemy, damage);
		}
	...
	public int defenseProc( Char enemy, int damage ) {
		...
		// 词条防御加成
		if (Dungeon.traits != null) {
			damage = Math.round(damage * (1 - TraitHandler.getDefenseBonus(this)));
			// 荆棘反伤
			int thorns = TraitHandler.onDefenseProc(this, enemy, damage);
			if (thorns > 0) {
	...
	public void move(int step, boolean travelling) {
		...
		// 步数触发问答（无 AI / 纯本地）
		if (Dungeon.quizSystem != null && Dungeon.quizSystem.onStep()) {
			try {
				final QuestionBank.Question q = Dungeon.quizSystem.getNextQuestion();
		...
		if (!flying && travelling) {
			// 词条移动触发（漂浮等）
			if (Dungeon.traits != null) {
				com.shatteredpixel.shatteredpixeldungeon.items.TraitHandler.onMoveProc(this);
			}
