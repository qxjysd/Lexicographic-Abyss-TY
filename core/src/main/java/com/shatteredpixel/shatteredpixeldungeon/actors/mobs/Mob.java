...
public abstract class Mob extends Char {
	...
	public void destroy() {
		...
		if (Dungeon.hero.isAlive()) {
			...
			if (alignment == Alignment.ENEMY) {
				...
				// Artifact on-kill hooks - grant small XP to equipped artifacts
				if (Dungeon.hero.buff(MagicImmune.class) == null) {
					for (KindofMisc miscItem : Dungeon.hero.belongings.misc) {
						if (miscItem != null && !miscItem.cursed) {
							if (miscItem instanceof HornOfPlenty) {
								((HornOfPlenty) miscItem).onEnemyKilled();
							} else if (miscItem instanceof MasterThievesArmband) {
								((MasterThievesArmband) miscItem).onEnemyKilled();
							}
						}
					}
				}
	...
	public void rollToDropLoot(){
		if (Dungeon.hero.lvl > maxLvl + 2) return;
				...
				MasterThievesArmband.StolenTracker stolenMBA = buff(MasterThievesArmband.StolenTracker.class);
				if (stolenMBA == null || !stolenMBA.itemWasStolen()) {
