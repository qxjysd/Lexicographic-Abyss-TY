...
public class Hunger extends Buff implements Hero.Doom {
	...
	public static final float STARVING	= 450f; //base, actual cap computed dynamically
	...
	public boolean act() {
		...
		if (target.isAlive() && target instanceof Hero) {
			...
			if (isStarving()) {
				...
				float hungerIncrease = (1f/hungerDelay) + Math.max(0, hero.STR - 10) * 0.5f;
				float newLevel = level + hungerIncrease;
	...
	public void affectHunger(float energy, boolean overrideLimits ) {
		...
		if (level < 0 && !overrideLimits) {
			...
			float excess = level - starvingThreshold();
	...
	public boolean isStarving() {
		return level >= starvingThreshold();
	}
	...
	//动态计算饱腹上限: 基础450 + 等级*10
	public float starvingThreshold() {
		if (target instanceof Hero) {
			return STARVING + ((Hero)target).lvl * 10;
		}
		return STARVING;
	}
