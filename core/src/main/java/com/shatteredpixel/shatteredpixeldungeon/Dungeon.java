...
import com.shatteredpixel.shatteredpixeldungeon.items.HeroTraits;
...
public class Dungeon {
	...
	public static HeroTraits traits = new HeroTraits();
	public static QuizSystem quizSystem = new QuizSystem();
	...
	public static Level newLevel() {
		...
		if (!(level instanceof DeadEndLevel || level instanceof VaultLevel)){
			...
			if (depth > Statistics.deepestFloor && branch == 0) {
				...
				Dungeon.traits.grantTrait( depth );
	...
	public static void saveGame( int save ) {
		try {
			...
			bundle.put( "quizSystem", quizSystem );
	...
	public static void loadGame( int save, boolean fullLoad ) throws IOException {
		...
		if (bundle.contains( "traits" )) {
			traits = (HeroTraits) bundle.get( "traits" );
		} else {
			traits = new HeroTraits();
		}

		Statistics.restoreFromBundle( bundle );
		if (bundle.contains( "quizSystem" )) quizSystem = (QuizSystem) bundle.get( "quizSystem" );
