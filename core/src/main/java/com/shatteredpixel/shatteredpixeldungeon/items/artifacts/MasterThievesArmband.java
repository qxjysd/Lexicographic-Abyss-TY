...
public class MasterThievesArmband extends Artifact {
				...
				if (ch instanceof Shopkeeper){
					...
					curUser.sprite.attack(target, new Callback() {
						...
						public void call() {
							...
							while (exp >= (10 + Math.round(3.33f * level()))) {
	...
	// Called when hero attacks an enemy - auto-steal attempt
	public void onHeroAttack(Hero hero, Char enemy, int damage) {
		if (cursed || hero.buff(MagicImmune.class) != null || charge <= 0) {
			return;
		}
		...
		if (mob.alignment != Char.Alignment.ENEMY
				&& !(mob instanceof Mimic && mob.alignment == Char.Alignment.NEUTRAL)) {
			return;
		}

		// Auto-steal attempt
		boolean surprised = mob.surprisedBy(hero, false);
		float lootMultiplier = 1f + 0.1f*level();
		int debuffDuration = 3 + level()/2;
		...
		if (Dungeon.hero.lvl > mob.maxLvl + 2) {
			lootChance = 0;
		} else if (mob.buff(StolenTracker.class) != null){
			lootChance = 0;
		}

		if (lootChance == 0) {
			// Nothing to steal - apply weakness debuff instead
			Buff.prolong(mob, Weakness.class, debuffDuration);
			// Still gain a bit of exp for the attempt
			exp += 1;
		} else if (Random.Float() <= lootChance){
			Item loot = mob.createLoot();
			if (Challenges.isItemBlocked(loot)){
				Buff.affect(mob, StolenTracker.class).setItemStolen(false);
				// Apply weakness on failed steal
				Buff.prolong(mob, Weakness.class, debuffDuration);
			} else {
				if (loot.doPickUp(hero)) {
					hero.spend(-loot.pickupDelay());
				} else {
					Dungeon.level.drop(loot, hero.pos).sprite.drop();
				}
				GLog.i(Messages.get(MasterThievesArmband.class, "stole_item", loot.name()));
				Buff.affect(mob, StolenTracker.class).setItemStolen(true);
			}
		} else {
			Buff.affect(mob, StolenTracker.class).setItemStolen(false);
			// Apply weakness on failed steal
	...
	// Called when hero kills an enemy - grants small XP to the artifact
	public void onEnemyKilled() {
		if (cursed || Dungeon.hero.buff(MagicImmune.class) != null) return;
