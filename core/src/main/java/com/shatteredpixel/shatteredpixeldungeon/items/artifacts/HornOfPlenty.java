...
public class HornOfPlenty extends Artifact {
	...
	public void doEatEffect(Hero hero, int chargesToUse){
		...
		// Satiety per charge scales with level - more food satisfaction at higher levels
		float levelBonus = 1f + level() * 0.1f;
		int totalSatiety = Math.round(satietyPerCharge * chargesToUse * levelBonus);

		Buff.affect(hero, Hunger.class).satisfy(totalSatiety);
		...
		Talent.onFoodEaten(hero, totalSatiety, this);
	...
	public void gainFoodValue( Food food ){
		// No level cap - always gain food energy
		...
		if (storedFoodEnergy >= Hunger.HUNGRY){
			...
			//no level cap, apply all earned upgrades
			upgrade(upgrades);
	...
	// Called when the hero kills an enemy - grants small XP to the artifact
