...
public class ChaliceOfBlood extends Artifact {
	...
	public void charge(Hero target, float amount) {
		...
		//healing scales continuously with level, no cap
	...
	// Called when the hero lands a hit on an enemy
	// Returns modified damage (with 30% lifesteal applied to the incoming damage)
	public int onHeroAttack(Hero hero, Char enemy, int damage) {
		if (cursed || hero.buff(MagicImmune.class) != null || damage <= 0) {
			return damage;
		}

		// 30% lifesteal: heal for 30% of damage dealt
		int healAmount = Math.round(damage * 0.3f);
		...
		// Absorb blood for experience (level-up)
